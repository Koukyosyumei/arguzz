...
class CircuitProjectGenerator(AbstractCircuitProjectGenerator):
    """
    Emits a Sphinx host+guest Rust project around the generated circuits.

    Sphinx layout (vs. SP1's `crates/...` paths):
      * SDK at  ``<repo>/sdk``           (crate ``sphinx-sdk``)
      * zkvm   ``<repo>/zkvm/entrypoint`` (crate ``sphinx-zkvm``)
      * helper ``<repo>/helper``         (crate ``sphinx-helper``,
                                          provides ``build_program(path)``)
    """
...
[dependencies]
sphinx-sdk = {{ path = "{self.zkvm_path}/sdk" }}
"""
        )
...
[build-dependencies]
sphinx-helper = {{ path = "{self.zkvm_path}/helper" }}
"""
        )
        create_file(
...
"""
    ...
    let stdin = create_sphinx_stdin"""
        ...
        buffer.write(
            f""";
    let (pk, vk) = client.setup(SPHINX_GUEST_ELF);
    println!(
        "<record>{{{{\\
            \\"context\\":\\"Setup\\", \\
            \\"status\\":\\"success\\", \\
...
[dependencies]
sphinx-zkvm = {{ path = "{self.zkvm_path}/zkvm/entrypoint" }}
""",
        )
    ...
    def create_guest_main_rs(self):
        ...
        buffer.write("sphinx_zkvm::entrypoint!(main);\n")
        ...
        for circuit in self.circuits:
            for parameter in circuit.inputs:
                ...
                buffer.write(
                    f"    let {var_name}: {var_type} ="
                    f" sphinx_zkvm::io::read::<{var_type}>();\n"
                )
        buffer.write("\n")

        def helper_commit_and_exit(value: int, is_end: bool) -> list[str]:
            if is_end:
                return [f"sphinx_zkvm::io::commit::<{RUST_GUEST_RETURN_TYPE}>(&{value}_u32);"]
            else:
                return [
                    f"sphinx_zkvm::io::commit::<{RUST_GUEST_RETURN_TYPE}>(&{value}_u32);",
                    "return; // abort",
                ]
